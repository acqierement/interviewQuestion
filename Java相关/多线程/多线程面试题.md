...
## 【todo】讲一下`ConcurrentHashMap`、`AtomicInteger`
...
## 成员变量i，用sychronized包括的i++线程安全吗【不懂】
...
## 【todo】程序出现死锁如何进行定位，如何解决

（答的不是很好，回答了用jstack定位代码，并应该尽量提前避免死锁，比如使用tryLock之类带时间限制的）

## 一个程序，while(true){new Thread().start()}运行会发生什么【不懂】
